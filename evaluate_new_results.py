...
def compute_item_popularity(train_df: pd.DataFrame) -> Dict[int, float]:
    ...
    return {int(item): float(prob) for item, prob in popularity.items()}


def build_user_item_matrix(train_df: pd.DataFrame) -> tuple[csr_matrix, Dict[int, int]]:
    if train_df.empty:
        return csr_matrix((0, 0)), {}
    user_ids = pd.Index(train_df["userId"].unique())
    item_ids = pd.Index(train_df["movieId"].unique())

    user_index = {int(user_id): idx for idx, user_id in enumerate(user_ids)}
...
def build_relevance(test_df: pd.DataFrame) -> Dict[int, set[int]]:
    if test_df.empty:
        return {}
    positive = test_df[test_df["rating"] > 0]
    grouped = positive.groupby("userId")["movieId"].apply(lambda items: set(int(x) for x in items))
    return {int(user): items for user, items in grouped.items()}


def deduplicate_preserve_order(items: Sequence[int]) -> List[int]:
    seen = set()
    deduped: List[int] = []
    for item in items:
        if item not in seen:
...
def build_recommendations(predictions_df: pd.DataFrame, top_k: int) -> Dict[int, List[int]]:
    ...
    for user_id, recs in recommendation_lists.items():
        cleaned = deduplicate_preserve_order([int(item) for item in recs])[:top_k]
        if cleaned:
            recommendations[int(user_id)] = cleaned
    return recommendations


def compute_ils(
    item_list: Sequence[int],
    similarity_matrix: sparse.spmatrix | np.ndarray,
    item_index_map: Mapping[int, int],
) -> float:
...
def evaluate_recommendations(
    ...
    relevant_items: Mapping[int, set[int]],
...
def prepare_dataset_artifacts(
    dataset_size: str,
    data_dir: Path,
) -> tuple[Dict[int, set[int]], Dict[int, float], Dict[int, int], sparse.spmatrix | np.ndarray] | None:
