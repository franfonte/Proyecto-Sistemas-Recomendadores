...
def train_model(train_df):
    """
    "Entrena" calculando la FRECUENCIA de cada película y normalizándola a una escala 1-10.
    Devuelve una serie de pandas con movieId como índice y el score de popularidad (1-10) como valor.
    """
    print("2. Calculando popularidad (Frecuencia normalizada a escala 1.0 - 10.0)...")
    ...
    # 2. Normalización Min-Max para forzar el formato de "Ratings" (1.0 a 10.0)
    ...
    if max_views == min_views:
        ...
        popularity_model = raw_counts.apply(lambda x: 10.0)
    else:
        # Fórmula: Scaled = 1 + (x - min) * (10 - 1) / (max - min)
        popularity_model = 1.0 + (raw_counts - min_views) * 9.0 / (max_views - min_views)
    ...
    print(f"   Modelo de popularidad creado. Item más visto escalado a 10.0, menos visto a 1.0.")
    return popularity_model, global_average

def generate_predictions(model, antitest_df):
    """
    Asigna el score de popularidad (escala 1-10) de cada película como la predicción.
    """
