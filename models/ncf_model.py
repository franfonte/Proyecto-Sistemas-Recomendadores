...
def train_model(training_components):
    ...
    # <<<<<<< CAMBIO AQUÍ: Detectar dispositivo disponible >>>>>>>
    ...
    # <<<<<<< CAMBIO AQUÍ: Mover modelo al dispositivo >>>>>>>
    ...
    for epoch in range(EPOCHS):
        ...
        for users, items, ratings in train_loader:
            # <<<<<<< CAMBIO AQUÍ: Mover datos al dispositivo en cada lote >>>>>>>
...
def generate_predictions(model, prediction_components):
    ...
    with torch.no_grad():
        for i, (users, items) in enumerate(pred_loader):
            ...
            predictions = model(users, items)
            # Mover las predicciones de vuelta a la CPU para convertirlas a numpy
