...
def load_metadata():
    ...
    try:
        ...
        id_to_title = pd.Series(df_movies.title.values, index=df_movies.movieId).to_dict()
        return id_to_title
...
def get_user_history_titles(user_id_str, df_ratings, id_to_title, limit=15):
    ...
    user_history = df_ratings[df_ratings['userId'] == user_id_str]
    ...
    top_history_ids = user_history['movieId'].head(limit).values
...
def main():
    ...
    if not os.path.exists(MOVIES_PATH) or not os.path.exists(RATINGS_PATH):
        print(f"❌ Error: No se encuentran los archivos CSV en 'data/ml-1m/'.")
        return
    ...
    df_ratings = pd.read_csv(RATINGS_PATH, usecols=['userId', 'movieId', 'rating'], dtype={'userId': str, 'movieId': str})
